...
dataframes_dict = read_csv(dataset_array, False)
top10 = st.sidebar.checkbox("Top-10", True)

# i = st.sidebar.slider("filter >=", 0, 10000, 100, step=100) if not top10 else 0
...
if dataframes_dict[dataset_key].loc[last_pos, "date"] < str(datetime.date.today()):
	st.write("Data is old.")
